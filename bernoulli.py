...
num_particulas = 200  #Número reduzido de partículas para otimizar performance
...
def calcular_energia_cinetica():
    return 0.5 * np.sum(np.linalg.norm(velocidades, axis=1)**2) #Ec ​= 1/2(​∑Mi​*Vi^2), Mi é a massa e Vi é a velocidade de cada partícula/

#Função para calcular pressão baseada na energia cinética e volume
def calcular_pressao():
    return (2/3) * (calcular_energia_cinetica() / volume_atual) #P = 2/3(Ec/V), Ec é a energia cinética e V é o volume do recipiente.
...
fig = plt.figure(figsize=(12, 4))  #Menor figura para máquinas menos potentes
...
ani = FuncAnimation(fig, atualizar, frames=np.arange(0, 5000), interval=50)  #Aumento no intervalo para suavizar a execução
plt.show()
